...
<form method="post" class="change-form">
    ...
    <p class="change-subtitle">Teacher Account Security</p>
    ...
    <div class="password-note">
        ...
        <a href="teach_profile.jsp" class="back-btn">
         Back
        </a>
...
if(old!=null)
{
    ...
    else
    {
        try
        {
            ...
            "select t_password from teacher where t_username=?");
            ...
            if(rs.next())
            {
                String dbpass=rs.getString("t_password");

                if(dbpass.equals(old))
                {

                    PreparedStatement ps2=con.prepareStatement("update teacher set t_password=? where t_username=?");

                    ps2.setString(1,np);
                    ps2.setString(2,t_username);

                    int x=ps2.executeUpdate();

                    if(x>0)
                    {
                        out.println("<script>alert('Password Updated Successfully');window.location='teach_profile.jsp';</script>");
                    }

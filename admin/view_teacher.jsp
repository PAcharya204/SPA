    ...
    <div id="main">
        <div class="table-container">
            ...
            <table class="analytics-table">
                ...
                %>
                <tr>
                    <td><img src="../<%=rs.getString("t_profile")%>" class="table-photo"></td>
                    <td><%=rs.getInt("tid")%></td>
                    <td><%=rs.getString("t_name")%></td>
                    <td><%=rs.getString("t_username")%></td>
                    <td><%=rs.getString("dept")%></td>
                    <td><%=rs.getString("t_dob")%></td>
                    <td><%=rs.getString("t_phone")%></td>
                    <td><%=rs.getString("t_gender")%></td>
                    <td><%=rs.getString("t_addr")%></td>
                    <td><a class="edit-btn"href="t_edit.jsp?id=<%=rs.getInt("tid")%>">Edit</a></td>
                    <td><a class="delete-btn"href="t_delete.jsp?id=<%=rs.getInt("tid")%>"onclick="return confirm('Are you sure you want to delete this teacher?')">Delete</a></td>
                </tr>
                <%
                    }
                    if(!found)
                    {
                        %>
                        <tr>
                            <td colspan="10" class="no-data">No Teachers Found</td>
                        </tr>
                        <%
                    }

...
<form method="post" class="change-form">
    ...
    <p class="change-subtitle">Principal Account Security</p>
    ...
    <div class="password-note">
        ...
        <a href="princi_profile.jsp" class="back-btn">
         Back
        </a>
...
if(old!=null)
{
    ...
    else
    {
        try
        {
            ...
            "select p_password from principal where p_username=?");
            ...
            if(rs.next())
            {
                String dbpass=rs.getString("p_password");

                if(dbpass.equals(old))
                {

                    PreparedStatement ps2=con.prepareStatement("update principal set p_password=? where p_username=?");

                    ps2.setString(1,np);
                    ps2.setString(2,p_username);

                    int x=ps2.executeUpdate();

                    if(x>0)
                    {
                        out.println("<script>alert('Password Updated Successfully');window.location='princi_profile.jsp';</script>");
                    }

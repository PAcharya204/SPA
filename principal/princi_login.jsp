...
if(username!=null)
{
    ...
    else
    {
        try
        {
            PreparedStatement ps= con.prepareStatement("select * from principal where p_username=? and p_password=?");
            ps.setString(1,username);
            ps.setString(2,password);
            ResultSet rs=ps.executeQuery();
            if(rs.next())
            {
                session.setAttribute("p_username",username);
                session.removeAttribute("captcha");
                out.println("<script>alert('Login Successful'); window.location='princi_dashboard.jsp';</script>");
            }
            else
            {
